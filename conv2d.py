...
class conv2d:
	...
	def forward(self, matrix):
		'''
		matrix - numpy array - img/matrix of 4D shape (batch_size, height, width,depth/channel)
		'''
		self.prev_matrix = matrix
		if(len(matrix.shape) != 4):
			raise ValueError("Your image dimensions need to be 4D (batch_size, height, width,depth/channel")
		if(self.filters is None):
			#(number of filters, kernel_size, kernel_size, depth/channel)
			k = 1/(matrix.shape[1] * self.kernel_size * self.kernel_size)
			self.filters = np.random.uniform(low=-math.sqrt(k), high=math.sqrt(k),size=(self.num_filters,self.kernel_size, self.kernel_size, matrix.shape[3]))
		...
		o_height = math.floor((matrix.shape[1] - self.kernel_size + 2*self.padding)/self.stride) + 1
		o_width = math.floor((matrix.shape[2] - self.kernel_size + 2*self.padding)/self.stride) + 1
		'''
		#invalid output size
		if(o_height.is_integer()):
			o_height = int(o_height)
		else:
			raise ValueError("Check your convolution output size! Excepted integer, got float")
		if(o_width.is_integer()):
			o_width = int(o_width)
		else:
			raise ValueError("Check your convolution output size! Expected integer, got float")'''

		o_depth = self.num_filters
		#zero pad image
		if(self.padding > 0):
			pad_shape = ((0, 0), (self.padding, self.padding), (self.padding, self.padding), (0, 0))
			matrix = np.pad(matrix, pad_shape, mode='constant', constant_values = (0, 0))
		self.prev_matrix = matrix
		batch_size = matrix.shape[0]
		#(batch_size,height, width,depth/channels)
		output = np.zeros((batch_size, o_height, o_width, o_depth))

		#iterate over each image in the batch
		for i in range(batch_size):
			for x in range(o_height):
				h_lower = x * self.stride
				h_upper = h_lower + self.kernel_size
				for y in range(o_width):
					w_lower =y * self.stride
					w_upper = w_lower + self.kernel_size
					for f in range(self.num_filters):
						output[i,x,y,f] = (np.sum(np.multiply(self.filters[:,:,:,f], matrix[i,h_lower:h_upper,w_lower:w_upper,:])))
						output += self.bias[f]
		return output
